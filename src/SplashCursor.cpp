...
namespace SplashNG {
    HRESULT BitmapFromLossless2Tag(
        DefineBitsLossless2Tag* db,
    ...
    HRESULT LoadLosslessBitmaps(
        ...
        for (Tag* t : tagList) {
            if (DefineBitsLossless2Tag* db = dynamic_cast<DefineBitsLossless2Tag*>(t)) {
                ...
                hr = BitmapFromLossless2Tag(db, pIWICImagingFactory, rt, pBitmap.GetAddressOf());
                if (SUCCEEDED(hr)) {
    ...
    SplashCursor::SplashCursor(std::string path, ComPtr<ID2D1DCRenderTarget> rt) {
        ...
        LoadLosslessBitmaps(swfParser.getTagList(), pIWICImagingFactory.Get(), rt.Get(), &losslessBitmaps);
        int i = 0;
        for (Tag* tag : tagList) {
        ...
        float scale)
    {
        ...
        hr = rt->CreateCompatibleRenderTarget(&size, nullptr, &pixelFormat, D2D1_COMPATIBLE_RENDER_TARGET_OPTIONS_NONE, &bitmapTarget);
        if(FAILED(hr)) {
        ...
        for (auto& [shapeTag, subShapeList] : shapeSubShapes) {
            for (auto& subShape : subShapeList) {
                ...
                case FILLSTYLE::SOLID:
                    ...
                    break;
                case FILLSTYLE::FOCAL_RADIAL_GRADIENT: //we're not handling gradients yet
