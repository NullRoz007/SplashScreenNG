...
SplashTextAlignment getTextAlignment() {
  ...
  try {
    result.textAlignment = static_cast<DWRITE_TEXT_ALIGNMENT>(
        Config::getFrom<int>("textAlignments", textAlignName, 2));
...
DWRITE_FONT_WEIGHT getTextWeight() {
  ...
  try {
    result = static_cast<DWRITE_FONT_WEIGHT>(
        Config::getFrom<int>("textWeights", weightName, 400));
  } catch (exception& e) {
...
DWRITE_FONT_STYLE getTextStyle() {
  ...
  try {
    result = static_cast<DWRITE_FONT_STYLE>(
        Config::getFrom<int>("textStyles", styleName, 0));
  } catch (exception& e) {
