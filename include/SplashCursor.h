...
class SplashCursor {
 public:
  SplashCursor(std::string path,
               Microsoft::WRL::ComPtr<ID2D1DCRenderTarget> rt);
  void CreateCursorBitmap(Microsoft::WRL::ComPtr<ID2D1Factory> factory,
                          Microsoft::WRL::ComPtr<ID2D1DCRenderTarget> rt,
                          float scale);

  void RenderCursor(Microsoft::WRL::ComPtr<ID2D1Factory> factory,
                    Microsoft::WRL::ComPtr<ID2D1DCRenderTarget> rt, float x,
                    float y);
 ...
 private:
  ...
  void RenderGradientToTarget(FILLSTYLE fillStyle, SubShape subShape,
                              ID2D1BitmapRenderTarget* rt,
                              ID2D1Factory* factory);
  void RenderSolidToTarget(FILLSTYLE fillStyle, SubShape subShape,
                           ID2D1BitmapRenderTarget* rt, ID2D1Factory* factory);
